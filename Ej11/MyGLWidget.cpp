...
void MyGLWidget::keyPressEvent(QKeyEvent* event)
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_L: { // canvia òptica entre perspectiva i axonomètrica
      if(whiteLight == 0) {
        whiteLight = 1.0f;
        emit cyanLightToggled(false);
        emit whiteLightToggled(true);
      } else {
        whiteLight = 0.0f;
        emit cyanLightToggled(true);
        emit whiteLightToggled(false);
      }
      glUniform1f(whiteLightLoc, whiteLight);
      break;
    }
...
void MyGLWidget::ToggleWhiteLight(bool _) {
    makeCurrent();
    whiteLight = 1.0f;
    glUniform1f(whiteLightLoc, whiteLight);
    emit cyanLightToggled(false);
...
void MyGLWidget::ToggleCyanLight(bool _) {
    makeCurrent();
    whiteLight = 0.0f;
    glUniform1f(whiteLightLoc, whiteLight);
    emit whiteLightToggled(false);

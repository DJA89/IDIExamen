...
MyGLWidget::MyGLWidget (QWidget* parent) : QOpenGLWidget(parent), program(NULL)
{
  ...
  secondCam = false; // ***** INICIALIZAMOS LA CAMARA *****
  hasToPaintZebra = 0.0; // ***** INICIALIZAMOS LA VARIABLE DE CEBRADO *****
  focusStatus = 0.0;
  quanity = 0.0; // ***** INICIALIZAMOS LA CANTIDAD QUE QUEREMOS DESPLAZAR ORIGINALMENTE *****
  colorSuelo = glm::vec3(0,0,0); // ***** INICIALIZAMOS EL COLOR DEL SUELO *****
}
...
void MyGLWidget::iniEscena ()
{
  ...
  radiEsc = glm::distance(glm::vec3(-2.8,-1.0,-2.1), glm::vec3(2.9,2.5,2.1))/2.0;
  // ***** INICIALIZAMOS LA POSICION DEL FOCO 2 *****
  glm::vec3 posFocus = glm::vec3((-2.0 + quanity), (-0.5 + 2.0) , 0.0);
  glUniform3fv(focus2Loc,1,&posFocus[0]);
}
...
void MyGLWidget::iniCamera ()
{
  ...
  vrp = glm::vec3(0,1.5,0); // ***** RECORDAMOS QUE EL SUELO ESTA EN LA POS -1 *****
...
void MyGLWidget::paintGL () 
{
  ...
  // ***** INDICAMOS QUE NO SE VA A PINTAR LA VACA *****
  glUniform1f(printVacaLoc,0.0f);
...
void MyGLWidget::createBuffersTerraIParet ()
{
  ...
  glm::vec3 diff = colorSuelo; // ***** COMPONENTE DIFUSA *****
  glm::vec3 spec(1.0,1.0,1.0);// ***** COMPONENTE ESPECULAR *****
...
void MyGLWidget::carregaShaders()
{
  ...
  // ***** LOCALIZAMOS DONDE ESTA LA VARIABLE PARA EL MATERIAL DE LA VACA *****
  printVacaLoc = glGetUniformLocation(program->programId(), "isPrintingVaca");
  // ***** LOCALIZAMOS LA VARIABLE DE PATRON CEBRADO *****
  paintZebraLoc = glGetUniformLocation(program->programId(), "paintingZebra");
  // ***** LOCALIZAMOS LA VARIABLE PARA CAMBIAR EL TIPO DE FOCO *****
  changeFocusLoc = glGetUniformLocation(program->programId(), "changeFocus");
  // ***** LOCALIZAMOS EL FOCO 2 *****
  focus2Loc = glGetUniformLocation(program->programId(), "posFocus2");
}

void MyGLWidget::modelTransformPatricio ()
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  // ***** AÑADIMOS EL MOVIMIENTO ADICIONAL *****
  TG = glm::translate(TG, glm::vec3(quanity,0.0,0.0));
  TG = glm::translate(TG, glm::vec3(-2.0, -0.5, 0.0));
  TG = glm:: rotate(TG, (float)M_PI/2.0f, glm::vec3(0,1,0));
  TG = glm::scale(TG, glm::vec3(escala/2.5f, escala/2.5f, escala/2.5f));
...
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::translate(TG, glm::vec3(2.0, 0.25, 1.0)); // ***** SUBIMOD EL PATRICIO *****
  TG = glm:: rotate(TG, -(float)M_PI/2.0f, glm::vec3(0,1,0)); // ***** LE DAMOS LA VUELTA *****
...
void MyGLWidget::projectTransform ()
{
  ...
  if(secondCam == true){ // ***** PARA LA SEGUNA CAMARA, EL ZN Y ZF LOS PONGO GRANDES PARA QUE NUNCA RECORTEN *****
       Proj = glm::perspective((float)M_PI/2.0f, ra, 0.1f, 20.0f);
  }
  else{
      if (perspectiva)
...
void MyGLWidget::viewTransform ()
{
  ...
  else { // ***** PARA LA SEGUNDA CAMARA *****
      View = glm::lookAt(glm::vec3((-2.0 + quanity), (-0.5 + 1.5) , 0.0),glm::vec3(2.0,0.25,1.0), glm::vec3(0,1,0));
  }
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_R: { // ***** YA NO ROTAMOS *****
      /*rotacionAdicional += (float)M_PI/6.0f;
      if(secondCam == true) viewTransform(); // **** NECESITAREMOS CAMBIAR EL LOOKAT PARA IR MIRANDO AL PATRICIO *****
      */break;
    }
  case Qt::Key_C: { // ***** VOLVEMOS A CAMBIAR DE CAMARA *****
    secondCam = !secondCam;
    viewTransform();
    projectTransform();
    camChanged();
  ...
  case Qt::Key_X: { // **** TRATAMOS EL PATRON CEBRADO *****
      if(hasToPaintZebra == 1.0) hasToPaintZebra = 0.0f;
      else hasToPaintZebra = 1.0f;
      glUniform1f(paintZebraLoc, hasToPaintZebra);
      break;
  }
  case Qt::Key_L: { // ***** CAMBIAMOS EL TIPO DE FOCO *****
    if(focusStatus == 0.0) focusStatus = 1.0;
    else focusStatus = 0.0;
    focusChanged(); // ***** CAMBIAMOS EL RADIO BUTTON *****
    glUniform1f(changeFocusLoc, focusStatus);
    break;
  }
  case Qt::Key_A: {
    quanity += 0.1;
    // ***** CALCULAMOS LA POSICION DEL FOCO Y LA ENVIAMOS AL SHADER *****
    glm::vec3 posFocus = glm::vec3((-2.0 + quanity), (-0.5 + 2.0) , 0.0);
    glUniform3fv(focus2Loc,1,&posFocus[0]);
    // ***** RECALCULAMOS LA VIEW PARA LA SEGUNDA CAMARA
    viewTransform();
    break;
  }
  case Qt::Key_D: {
...
void MyGLWidget::changeFocus(){ // ***** CAMBIAMOS EL FOCO CON EL RADIO BUTTON *****
    makeCurrent();
    if(focusStatus == 0.0) focusStatus = 1.0;
    else focusStatus = 0.0;
    glUniform1f(changeFocusLoc, focusStatus);
...
void MyGLWidget::changeCam(){ // *****  CAMBIAMOS LA CAMARA CON EL BOTON *****
    makeCurrent();
    secondCam = !secondCam;
    viewTransform();
    projectTransform();
...
// ***** CAMBIAMOS EL COLOR DEL SUELO Y EN CONSEQUENCIA HEMOS DE RECARGAR LOS BUFFERS *****
// ***** LOS SLIDERS HAN SIDO CAMBIADOS PARA QUE DEN VALORES DE 0 A 255 COMO EN RGB *****
void MyGLWidget::changeRed(int cantidad){
    makeCurrent();
    colorSuelo[0] = (float)cantidad/255.0;
    createBuffersTerraIParet();
    update();
}

void MyGLWidget::changeGreen(int cantidad){
    makeCurrent();
    colorSuelo[1] = (float)cantidad/255.0;
    createBuffersTerraIParet();
    update();
}

void MyGLWidget::changeBlue(int cantidad){
    makeCurrent();
    colorSuelo[2] = (float)cantidad/255.0;
    createBuffersTerraIParet();
    update();
}

...
{
  radiEsc = glm::distance(glm::vec3(-2.0 - ancho1/2.0, -1.0, -2.0), glm::vec3(2.0 + ancho2/2.0, 1.5, 2.0))/2;
  ra = 1.0f;
  FOV_INI = FOV = 2 * asin(1.0/2.0);
  //FOV = float(M_PI/3.0);
}
...
void MyGLWidget::carregaShaders()
{
  ...
  TranspFocLoc = glGetUniformLocation (program->programId(), "TranspFoc");
}

void MyGLWidget::modelTransformPatricio1 ()
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::translate(TG, glm::vec3(-2 + movXPtr1, -0.5, 0));
...
void MyGLWidget::viewTransform ()
{
  ...
  View = glm::translate(View, -glm::vec3((ancho2-ancho1)/4, 0.25, 0));
...
void MyGLWidget::calculaCapsaModel ()
{
  ...
  float ancho = maxz - minz;
  ancho1 = escala1 * ancho;
  ancho2 = escala2 * ancho;
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_A: {
      if (movXPtr1 < 4) {
          movXPtr1 += 1.0;
          glUniform1f(TranspFocLoc, movXPtr1);
      }
      break;
    }
    case Qt::Key_D: {
